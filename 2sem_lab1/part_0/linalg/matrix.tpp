...
Matrix<T>::Matrix(std::initializer_list<std::initializer_list<T>> init)
    : cols_(0), rows_(init.size()) {   // init is two-dimensional array
    if (init.size() != 0) {
...
}  // convenient ctor(to do like this:Matrix<int> m{{1, 0, 2, -1}, {3, 0, 0, 5}, {2, 1, 4, -3}, {1,

// 0, 5, 0}}

template <typename T>
Matrix<T>::Matrix(unsigned rows, unsigned cols, T value) : cols_(cols), rows_(rows) {
    data_ = subvector<T>(rows_ * cols_, value);
}

template <typename T>
Matrix<int> Matrix<T>::Identity(unsigned rows, unsigned cols) {
...
template <typename T>
Matrix<T> Matrix<T>::getSpecificDeterminant(unsigned int n, T determinant) {
    Matrix result(n, n);
...
{
    // проверка requires std::integral<T> нужна для того, чтобы компилятор понимал, когда мы
    // вызываем этот метод для целочисленного типа то надо вызывать именно этот метод, а не
    // предыдущий. Без проверки код не скомпилируется
    ...
    for (T i = 3; i <= std::sqrt(determinant_copy);
         i += 2) {  // на кратность 2 проверили - идём через 2
...
template <typename T>
void Matrix<T>::shuffle(std::mt19937& gen) {
...
template <typename T>
Matrix<T> Matrix<T>::eliminate_row_col(unsigned int elim_row, unsigned int elim_col) {
...
// функция ниже более быстрая, приводит матрицу к верхнетреугольному виду и считает произведение
// элементов на главной диагонали. Чтобы не переусложнять решено сделать ее только для тех типов,
// которые приводятся к double.
...
template <typename U>
Matrix<U> operator*(const U& value, const Matrix<U>& rhs)
